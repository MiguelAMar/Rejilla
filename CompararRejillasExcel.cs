...
namespace demorejilla
{
    ...
    internal class CompararRejillasExcel
    {
        ...
        public void compararRejillasExcel(List<DataGridViewRow> conjunto)
        {
        ...
        private void exportarHojaCalculoNueva(DataGridViewRow filaActual, DataGridViewRow filaSiguiente, SheetData hojita1, SheetData hojita2, int filaHoja1, int filaHoja2)
        {
            ...
            if (filaSiguiente != null)
            {
                ...
                if (hayTiemposLatenciaActual)//miro si hay tiempos latencia siguientes y me los traigo
                {
                    tiemposEmparejamientoSiguiente = filaSiguiente.Cells[POS_TiemposLatencia].Value.ToString();
                    if (!string.IsNullOrEmpty(tiemposEmparejamientoSiguiente))
